...
2021/06/08 CHING @ NTU
add bourger anomaly into output file
'''
...
def compute_gravity(frame):
    ...
    print('gravity evaluated at %f m' % pz_height)
...
def compute_bg_gravity(frame,sea_rho=1030):
    ''' 
        bourger anomaly from Turcotte & Schubert, 1st Ed., Eq 5-110
        use interploate of surface element's density
    '''
    G=6.6726e-11
    fl = flac.Flac()
    end = fl.nrec
    ...
    def find_trench_index(z):
        ...
        i = zz[:imax].argmin()
        return imax,i
    arc,trench=find_trench_index(z)
    arc *= 5
    px, topo, fa_gravity = compute_gravity(frame)
    rock_rho = np.interp(px/1000, ele_x[:,0], dtop)
    gb_gravity = np.zeros(len(fa_gravity))
    for h in range(len(fa_gravity)):
        if topo[h]>=0 or h > arc :
            gb_gravity[h]=fa_gravity[h]-2*np.pi*rock_rho[h]*G*topo[h]
        if topo[h]<0 and h < arc :
            gb_gravity[h]=fa_gravity[h]-2*np.pi*(rock_rho[h]-sea_rho)*G*topo[h]

    return gb_gravity

def compute_gravity2(frame):
    '''Similar to compute_gravity(), but also return modified topo'''
    px, topo, fagrav = compute_gravity(frame)
    topomod = modify_topo(topo)
    bggrav = compute_bg_gravity(frame)

    return px, topo, topomod, fagrav, bggrav


if __name__ == '__main__':
    frame = int(sys.argv[1])

    px, topo, topomod, fagravity, bggravity = compute_gravity2(frame)
    flac.printing(px, topo, topomod, fagravity, bggravity)
